...
def read_68points(filename):
    with open(filename) as f:
        lines = f.readlines()
        lines = [line.split() for line in lines]
    points = [(float(l[0]), float(l[1])) for l in lines]
    return points
...
if __name__ == '__main__':
    ...
    def visualize_points(filenames):
        ...
        pts2 = read_68points(os.path.join(images_dir, imgfile+'.pts'))
        ...
        for i in range(num_pts2):
            xi, yi = pts2[i]
            cv2.circle(img, tuple(np.round([xi-1, yi-1]).astype('int32')), 2, (0,0,255), -1)
